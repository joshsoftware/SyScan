...
def generate_paired_data_from_fixer_preds_for_BIFI(pred_dir_prefix, pred_fname, out_dir):
    ...
    train_data_0['id']   = [line.strip() for line in tqdm(open('/content/drive/MyDrive/syscan/data/round0/data_paired/train.id'))]
    ...
    with open(f'{out_dir}/train.good', 'a') as fgood:
      for idx in tqdm(idxs):
        _prefix, _idx = idx.split('_')
        _idx = int(_idx)
        if _prefix == '0':
          stringa = linecache.getline('/content/drive/MyDrive/syscan/data/round0/data_paired/train.good', _idx)
          stringb = stringa.strip()
          fgood.write(stringb +'\n')
        else:
          fgood.write(train_data['good'][_idx] +'\n')
    fgood.close()
    ...
    with open(f'{out_dir}/train.bad', 'a') as fbad:
      for idx in tqdm(idxs):
        _prefix, _idx = idx.split('_')
        _idx = int(_idx)
        if _prefix == '0':
          stringa = linecache.getline('/content/drive/MyDrive/syscan/data/round0/data_paired/train.bad', _idx)
          stringb = stringa.strip()
          fbad.write(stringb +'\n')
        else:
          fbad.write(train_data['bad'][_idx] +'\n')
    ...
    os.system('cp {} {}'.format('/content/drive/MyDrive/syscan/data/round0/data_paired/dev.bad', out_dir))
    os.system('cp {} {}'.format('/content/drive/MyDrive/syscan/data/round0/data_paired/dev.good', out_dir))
    os.system('cp {} {}'.format('/content/drive/MyDrive/syscan/data/round0/data_paired/dev.id', out_dir))
    print ('done')
